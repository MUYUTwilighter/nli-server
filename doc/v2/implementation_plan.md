...
- v2 先在独立数据库、独立 Redis、独立进程/端口运行；不与 v1 共用写模型；
- 生产 Gate E/F 路由按文档要求成组开放，不能把缺失路由伪装为 94 项已完成；
- 切流采用 DNS/反向代理到已验证 v2 版本；观测期保留上一个已验证 v2 二进制和数据库恢复点；
- 停止 v1 写入是明确不可逆点。之后只能关闭功能、回退到兼容当前 v2 Schema 的前一 v2 版本或 forward-fix；
- v1 代码、测试和部署样例只在 v2 全量观测期通过后删除。
...
### Phase 10：Canary、全量切流与v1移除

拆成四个独立发布Gate：

1. **Hidden Relay Gate**：代码和内部Adapter部署但Relay关闭；
2. **Client Gate**：参考客户端与真实验收通过；
3. **Traffic Gate**：v2独立数据库/Redis备份恢复演练后，canary→全量切流并完成观测期；
4. **Removal Gate**：最后删除v1代码、旧migration装配、旧测试和coturn/nginx v1模板。

**切流前**：冻结写入窗口、确认没有v1数据迁移承诺、记录DNS/代理配置、保留前一兼容v2二进制和恢复点。

**切流后恢复**：关闭Relay/Signaling或受影响路由组；回退到兼容当前v2 Schema的前一v2版本；必要时数据库恢复并forward-fix。禁止重新开启v1写入。
...
| I-CUTOVER-01 | v1无数据迁移且切流不可逆 | 用户确认新系统空数据启动、备份恢复和forward-recovery演练通过 |
