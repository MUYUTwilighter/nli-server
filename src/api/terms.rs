...
When a runtime instance is registered, the service sends the supplied Minecraft access token to the official Minecraft profile service to verify the account. The token is not persisted. It may be submitted once more when you explicitly request synchronization of an official friend removal, and is discarded after verification.

The service may persist Minecraft profile UUIDs, cached profile names, NetherLink friendships, friend requests, their sources, and timestamps. Runtime instance tokens, presence entries, rate-limit counters, and signaling-session state are temporary data stored for service operation. Presence information may include profile and presence identifiers, status, display text, joinability, and update or expiry times, and is shared with accepted friends.
...
3. 服务使用的信息
注册运行实例时，服务会将提交的 Minecraft 访问令牌发送至 Minecraft 官方档案服务，以验证账号身份。该令牌不会被持久化。仅当你明确要求同步删除官方好友时，令牌可能被再次提交并在验证后丢弃。

服务可能持久化 Minecraft 档案 UUID、缓存的档案名称、NetherLink 好友关系、好友申请、数据来源及时间戳。运行实例令牌、Presence 在线状态、频率限制计数器和信令会话状态是为服务运行而保存的临时数据。Presence 可能包含档案与 Presence 标识符、状态、显示文本、是否允许加入以及更新或过期时间，并会提供给已接受的好友。
